...
Unable to build the Docker native images of `user-service` and `event-service`. The building process gets stuck while registering reflection metadata.

The image below shows part of the logs, the time spending and the machine resources consuming
![native-compilation-issue](images/native-compilation-issue.png)
