...
class EndToEndTestApplicationTests extends AbstractTestcontainers {
  ...
  void testGetUsersWhenThereIsNone() {
    ...
    ResponseEntity<UserResponse[]> responseEntity =
        testRestTemplate.getForEntity(userServiceUrl, UserResponse[].class);
  ...
  void testGetUserWhenNonExistent() {
    ...
    ResponseEntity<MessageError> responseEntity =
        testRestTemplate.getForEntity(userServiceUrl, MessageError.class);
  ...
  void testCreateUser() {
    ...
    String userServiceUrl = String.format("%s/users", USER_SERVICE_API_URL);
    ResponseEntity<UserResponse> responseEntity =
        testRestTemplate.postForEntity(userServiceUrl, createUserRequest, UserResponse.class);
        ...
        .untilAsserted(
            () -> {
              ...
              String eventServiceUrl =
                  String.format("%s/events?userId=%s", EVENT_SERVICE_API_URL, 1L);
  ...
  void testUpdateUser() {
    ...
    ResponseEntity<UserResponse> responseEntity =
        testRestTemplate.exchange(
            userServiceUrl, HttpMethod.PUT, requestUpdate, UserResponse.class);
    ...
    await()
        ...
        .untilAsserted(
            () -> {
              log.info("Waiting for event-service to receive the message and process ...");
              String eventServiceUrl =
                  String.format("%s/events?userId=%s", EVENT_SERVICE_API_URL, userId);
              ResponseEntity<UserEventResponse[]> eventServiceResponseEntity =
                  testRestTemplate.getForEntity(eventServiceUrl, UserEventResponse[].class);
              assertThat(eventServiceResponseEntity.getStatusCode()).isEqualTo(HttpStatus.OK);
              assertThat(eventServiceResponseEntity.getBody()).isNotNull();
  ...
  void testUpdateUserWhenNonExistent() {
    ...
    ResponseEntity<MessageError> responseEntity =
        testRestTemplate.exchange(
            userServiceUrl, HttpMethod.PUT, requestUpdate, MessageError.class);
  ...
  void testDeleteUser() {
    ...
    ResponseEntity<Void> responseEntity =
        testRestTemplate.exchange(userServiceUrl, HttpMethod.DELETE, null, Void.class);
  ...
  void testDeleteUserWhenNonExistent() {
    String userServiceUrl = String.format("%s/users/%s", USER_SERVICE_API_URL, 2L);
    ResponseEntity<MessageError> responseEntity =
        testRestTemplate.exchange(userServiceUrl, HttpMethod.DELETE, null, MessageError.class);
